    ...
    private fun decodeH264() {
        mediaCodec?.apply {
            ...
            while (true) {
                ...
                //查询哪个输入队列的buffer能够使用
                val inIndex = dequeueInputBuffer(10000)
                if (inIndex >= 0) {
                    //找到了
                    ...
                    queueInputBuffer(inIndex, 0, nextFrameStart - startIndex, 0, 0)
                    //标记下一帧的开始index
                ...
                //2、处理解码后的数据
                //取出解码后的数据包
                val outIndex = dequeueOutputBuffer(info, 10000)
                if (outIndex >= 0) {
                    //如果解码后的数据包有效，则渲染到传入的surface中
                    releaseOutputBuffer(outIndex, true)
                }
            }
        }
    }
